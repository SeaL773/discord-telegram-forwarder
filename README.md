...
Telegram does not expose a Topic state query suitable for reconciliation. On the first upgrade, enabled Topics are recorded as the baseline without issuing 136 reopen calls. Therefore manual Telegram-side changes can drift from local state; use this rules-driven workflow rather than manually closing Topics. Successful API state is local evidence, not an authoritative Telegram query result.
