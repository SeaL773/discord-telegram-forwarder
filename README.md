...
Docker Compose 2.35.0 or newer is required. The Compose file uses long-form `env_file.required`, and the documented secret-safe validation command relies on `config --no-env-resolution`.
...
Discord message and embed-description Markdown headings (`#` through `######`), bold, italic, and bold-italic emphasis are converted to Telegram HTML. User text is HTML-escaped before these supported markers are transformed; unsupported Markdown remains literal. `telegram.rich_messages_enabled` is a strict boolean and defaults to `false` in code. When enabled, every message uses Bot API 10.2 Rich Messages when its downloaded media is empty or entirely photo/video; unsupported document media and definite Rich API rejections retain the classic fallback path. Rich media is uploaded in one multipart `sendRichMessage` request and is never delegated to Telegram by Discord URL.
...
State is one mode-0600 atomic `/data/state.json` in the private mode-0700 Compose volume. First-start bootstrap order, forwarding/rejection payloads, exact frozen target/drop decisions, next index, ready boundary, in-flight target phase (`rich` → `media` → `fallback`), and successful synchronization states for currently managed Topics are durable. A persisted rich target resumes rich delivery even if the feature gate is later disabled. Rate limits do not consume retry budget. Ambiguous rich network/timeout/5xx/malformed-success outcomes retry only rich and dead-letter on exhaustion; they never cross-format fallback. Only definite 400/404/413 Rich Message rejection durably enters classic media, while 401/403 dead-letters directly. Legacy media/fallback state remains loadable. Ordinary, rich, and fallback retry counters are durable; each increment is persisted, and a persistence failure rolls the in-memory increment back. Topic-state pruning does not alter cursor, bootstrap, or in-flight recovery data. Recovery finishes persisted in-flight and bootstrap work before opening Bridge, so later rule changes cannot alter those already-frozen decisions; disabling a rule is not a retroactive cancellation of durable work. Target dead letters and prepare-stage rejection dead letters carry stable identities: if a process stops after a dead-letter append is fsynced but before terminal cursor state is persisted, recovery or replay scans the active dead-letter file and retained rotations, completes state without appending a duplicate record, and leaves other targets independent. This is a recoverable cross-file protocol, not a claim of atomicity across the state and dead-letter files.

### Privacy and retention

- `/data/state.json` can temporarily contain complete Discord event payloads for forwarding/rejected bootstrap items and in-flight recovery. Bootstrap items preclassified as drops store only their cursor and drop action.
- `/data/failed-events.ndjson` contains complete failed events and Telegram destination IDs. Before an append would make the active file exceed `state.dead_letter_max_bytes`, the forwarder writes and fsyncs the complete record to a temporary file, then atomically publishes it as `.pending`; an interrupted temporary write is discarded at startup and never promoted. Rotation persists the transaction's original backup count in a plan file, stages the active file and retained backups under private names, fsyncs a ready marker, promotes staged generations to `.1` through `.N`, and finally atomically promotes `.pending` to active. A configuration change during recovery completes the transaction with its persisted count, then applies the current retention count. Defaults are 32 MiB and two backups, so steady-state retained storage is at most three generations and approximately 96 MiB total; during an interrupted rotation, private temporary, stage, plan, ready, and pending files can exist temporarily. A single record larger than 32 MiB may make the newly created active file exceed that approximation because records are never silently truncated or redacted.
- Rotation uses same-directory replace operations, mode-0600 files, parent-directory fsync after every namespace change, and regular-file/symlink checks. `StateStore` startup completes an interrupted staged rotation before loading recovery state or scanning identities. Identity scanning remains bounded even when a complete target record exceeds the normal recovery-line parser limit because target and prepare-stage identities are serialized before the full payload.
...
Except for bootstrap items already frozen as cursor-only drops, forwarding/rejected bootstrap payloads and in-flight payloads remain in `state.json` until their ordered recovery step is durably completed. Those payloads are not minimized earlier because doing so would remove the frozen event/decision data required to preserve current ordered recovery semantics.
...
Do not run plain `docker compose config` in captured terminals or CI logs: it can render values loaded from `.env` or service `env_file` entries. The command above performs structural validation without printing the resolved application model; verify the flags are available with `docker compose config --help`.
