...
- **cursor 推进原则(关键)**: `last_acked_cursor` 只在事件**成功发送到 Telegram 之后**(或被规则明确 drop 之后)才推进。这样进程崩溃/重启后,从上次 ack 点 replay,实现 at-least-once。
...
- 配置热重载: 监听 `rules.yaml` mtime,变更后原子替换规则集,无需重启容器。
...
- 从事件里的 Discord CDN URL 下载(httpx,超时 15s,大小上限默认 20 MiB —— Bot API 上传上限 50 MiB,留余量)。
...
rules:
  - name: "trading-signals 全事件 → 交易信号频道"
    match:
      ...
      channel_id: ["222222222222222222", "333333333333333333"]
    forward_to:
...
| Bridge buffer 淘汰(停机过久) | replay 有缺口;日志 ERROR 告警,可从 NDJSON 手动补 |
| TG 429 | 按 `retry_after` 等待重试,队列自然背压 |
| 单条消息发送连续失败 | 进死信文件,cursor 继续推进,不阻塞流水线 |
| 媒体下载失败 | 降级为文本 + 链接 |
| Discord/Windows 整体离线 | Bridge 无事件,forwarder 空转;恢复后 collector NDJSON 续写,自动跟上 |

可观测性(最小集):
- Docker healthcheck: 进程内简单 HTTP `/healthz`(报告 WS 连接状态 + 最后事件时间)。
- 心跳自监控: 若 WS 断连超过 N 分钟,用 bot 向管理群(`<TELEGRAM_ADMIN_CHAT_ID>`,与其他 monitor 一致)发一条告警。
